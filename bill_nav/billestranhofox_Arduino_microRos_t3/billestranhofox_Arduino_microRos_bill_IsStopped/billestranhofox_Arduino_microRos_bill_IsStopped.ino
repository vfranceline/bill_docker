...
int roda_Dianteira_tickPerRevolution_RW = 663; //te[orico jurema //Original 1055;
//roda_Traseira
...
rcl_publisher_t isStopped_publisher;
...
volatile long last_encoder_count_L = 0;
volatile long last_encoder_count_R = 0;

nav_msgs__msg__Odometry odom_msg;
std_msgs__msg__Bool isStopped_msg;
rcl_timer_t timer;
...
void MotorControll_callback(rcl_timer_t* timer, int64_t last_call_time) {
  ...
  // 1) Leia contagens atuais de encoder das rodas dianteiras
  long curr_L = roda_Dianteira_leftWheel.EncoderCount.data;
  long curr_R = roda_Dianteira_rightWheel.EncoderCount.data;

  // 2) Se não mudou nenhuma das duas, está parado
  bool stopped = (curr_L == last_encoder_count_L) && (curr_R == last_encoder_count_R);

  // 3) Prepare e publique a mensagem
  isStopped_msg.data = stopped;
  RCSOFTCHECK(rcl_publish(&isStopped_publisher, &isStopped_msg, NULL));

  // 4) Atualize o last_* para próxima iteração
  last_encoder_count_L = curr_L;
  last_encoder_count_R = curr_R;
